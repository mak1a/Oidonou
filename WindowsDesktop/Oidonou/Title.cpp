
# include "Title.hpp"

Title::Title(const InitData& init)
    : IScene(init)
    , m_startButton(Arg::center(Scene::Center().movedBy(0, 0)), 300, 60)
    , m_exitButton(Arg::center(Scene::Center().movedBy(0, 100)), 300, 60)
    , m_startTransition(0.4s, 0.2s)
    , m_exitTransition(0.4s, 0.2s) {

}

void Title::update() {
    m_startTransition.update(m_startButton.mouseOver());
    m_exitTransition.update(m_exitButton.mouseOver());

    if (m_startButton.mouseOver() || m_exitButton.mouseOver()) {
        Cursor::RequestStyle(CursorStyle::Hand);
    }

    if (m_startButton.leftClicked()) {
        changeScene(State::Game);
    }

    if (m_exitButton.leftClicked()) {
        System::Exit();
    }
}

void Title::draw() const {
    const String titleText = U"Ç®Ç¢Ç«ÇÃÇ§ÉQÅ[ÉÄ";
    const Vec2 center(Scene::Center().x, 120);

    TextureAsset(U"River").resized(1400, 830).drawAt(Scene::CenterF());

    TextureAsset(U"Schoolkun").resized(400.f, 300.f).drawAt(Vec2(1230.f, Scene::CenterF().movedBy(0, 120).y).movedBy(-100.f, -100.f));

    FontAsset(U"Title")(titleText).drawAt(center.movedBy(4, 6), ColorF(0.0, 0.5));
    FontAsset(U"Title")(titleText).drawAt(center);

    m_startButton.draw(ColorF(1.0, m_startTransition.value())).drawFrame(2);
    m_exitButton.draw(ColorF(1.0, m_exitTransition.value())).drawFrame(2);

    FontAsset(U"Menu")(U"ÇÕÇ∂ÇﬂÇÈ").drawAt(m_startButton.center(), ColorF(0.25));
    FontAsset(U"Menu")(U"Ç®ÇÌÇÈ").drawAt(m_exitButton.center(), ColorF(0.25));

    Rect(0, 500, Scene::Width(), Scene::Height() - 500)
        .draw(Arg::top = ColorF(0.0, 0.0), Arg::bottom = ColorF(0.0, 0.5));

    const int32 highScore = getData().score;
    FontAsset(U"Score")(U"ëOâÒÇÃscore: {}"_fmt(highScore)).drawAt(Vec2(620, 550));
}
